...
setwd("/project/modelscape/analyses/sparse/data/simulations/sim_round5/")

datafiles<-list.files("/project/modelscape/analyses/sparse/data/simulations/sim_round5/", pattern="scenario*", recursive=TRUE, include.dirs=TRUE)

### there are 36 possible simulations
...
R2_data<-cbind.data.frame(scenario_rep[1:3600,], R2)
names(R2_data)<-c("scenario", "rep", "R2")
write.table(R2_data, file="/project/modelscape/analyses/sparseillustrateeval_bitbucket/analyses/output/sparse_R2vstheworld_out.txt", row.names=F, quote=F)

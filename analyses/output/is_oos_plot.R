...
## JPJ 14 vi 23
## Purpose: to plot in-sample vs out-of-sample prediction
...
uniq_metrics <- c("IS R2", "OOS Prediction R2")
uniq_metric_cols <- c(7,5)
...
pdf("is_oos_plot.pdf", height=8, width=18)
...
for (i in 1:dim(scenarios)[1])
	...
	if (scenarios[i,6]==0.1)
		{
		rect(par('usr')[1], par('usr')[3], 0.09732011, 0.09732011, col="light gray", border=NA)
		abline(h=0.09732011, lty=2)
		abline(v=0.09732011, lty=2)
		}
	else if (scenarios[i,6]==0.3)
		{
		rect(par('usr')[1], par('usr')[3], 0.4715198, 0.4715198, col="light gray", border=NA)
		abline(h=0.4715198, lty=2)
		abline(v=0.4715198, lty=2)
		}
	else if (scenarios[i,6]==0.8)
		{
		rect(par('usr')[1], par('usr')[3], 0.858247, 0.858247, col="light gray", border=NA)
		abline(h=0.858247, lty=2)
		abline(v=0.858247, lty=2)
		}
		abline(0,1)
		box(lwd=1)
	...
	if (i==32) { mtext(bquote("In-sample R"^2), side=1, line=4, cex=1.5) }
	if (i==10) { mtext(bquote("Out-of-sample R"^2), side=2, line=3, cex=1.5, adj=2.6) }

	for (j in 1:length(uniq_analyses))
		{
		points(plot_mat[plot_mat[,1]==i & plot_mat[,2]==uniq_analyses[j] & plot_mat[,3]=="IS R2",4], plot_mat[plot_mat[,1]==i & plot_mat[,2]==uniq_analyses[j] & plot_mat[,3]=="OOS Prediction R2",4], pch=21, bg=plot_colors[j], cex=2.5)
		}
	}
dev.off()

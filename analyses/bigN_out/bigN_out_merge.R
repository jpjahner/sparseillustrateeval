...
## JPJ 31 vii 23
## PURPOSE: to merge the prediction and parameter metrics from all bigN output files into one matrix
## RUN THIS FROM: /project/modelscape/analyses/sparseillustrate_bitbucket/analyses/bigN_out/
## USAGE: Rscript bigN_out_merge.R


## IMPORTANT: must change the date below to match to the corresponding trial you are merging

date <- "17dec23"
trial_path <- paste0("/project/modelscape/analyses/sparseillustrateeval_bitbucket/analyses/trials_out_bigN/", date, "/")


## set up the output matrix with the first 3 columns filled (scenario, rep, analysis)

scenarios <- c("bigN_scenario1", "bigN_scenario2")
reps <- c(1:100)
analyses <- c("lasso","glasso","ridge","gridge","elastic","gemma","susie","spike_slab","forest")
...
write.table(metrics_out, file=paste0("bigN_metrics_out_", date, ".txt"), row.names=FALSE, quote=FALSE)
